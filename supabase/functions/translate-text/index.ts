...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Translate every item below from Portuguese into ${targetName}. Treat each item as regular text content, NOT as a brand name — even single words like "Titulo", "Título", "Exemplo", "Produto", "Teste", "Promoção", "Oferta", "Categoria", "Cápsulas", "Comprimidos", "Frasco", "Caixa", "Suplemento", "Vitamina", "Disponibilidade", "Em Estoque", "Catálogo", "Exclusivo" MUST be translated. Accept misspellings or missing accents (e.g. "Titulo" = "Título"). Preserve casing (ALL CAPS stays ALL CAPS, Title Case stays Title Case). Only keep untouched: registered pharmaceutical/commercial brand names (e.g. "Cialis", "Viagra", "Tylenol", "Liberty Pharma"), SKU codes, dosages (e.g. "5mg", "10ml"), pure numbers, and clearly non-translatable proper nouns (e.g. person names "Alpha", "Beta" only when appearing alongside a brand). When in doubt, TRANSLATE. Return ONLY a JSON array of strings in the same order, no comments, no markdown fences.\n\nITEMS:\n${JSON.stringify(texts)}`;
